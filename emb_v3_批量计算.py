...
# 结果输出Excel文件名
OUTPUT_EXCEL = "similarity_result0427.xlsx"
...
def load_sdg_embeddings(sdg_folder, stopwords):
    ...
    if os.path.exists(main_sdg_path):
        ...
        sdg_embeddings['total'] = get_document_embedding_with_retry(main_sdg_text)
    ...
    for filename in os.listdir(sdg_folder):
        if filename.startswith("sdg - 副本 (") and filename.endswith(".txt"):
            try:
                ...
                sdg_embeddings[sdg_key] = get_document_embedding_with_retry(sdg_text)
            except Exception as e:
...
def process_pifu_files(pifu_folder, sdg_embeddings, stopwords):
    """处理批复文件夹中的所有文件，与所有SDG文件计算相似度"""
    # 准备结果列表
    results = []
        ...
        if filename.endswith('.txt'):
            ...
            try:
                ...
                for sdg_name, sdg_embedding in sdg_embeddings.items():
                    ...
                    print(f"{filename} 与{sdg_name}的余弦相似度为: {similarity:.4f}")
...
def save_to_excel(results, output_folder, output_filename):
    ...
    if results:
        # 获取所有可能的sdg列名并按数字排序
        sdg_columns = [col for col in results[0].keys() if col != 'city']
        # 确保total在最前面
...
def main():
    try:
        ...
        # 处理所有批复文件
        results = process_pifu_files(PIFU_FOLDER, sdg_embeddings, stopwords)
